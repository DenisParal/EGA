...
struct item{
    ...
    void print(){
        std::cout <<"Item nomber "<<index<<"; weight: "<<weight<<"; value: "<<value<<" cpec_value: "<<(double)(value)/(double)(weight)<<"\n";
    }
...
int main(){
    ...
    for(int i=0;i<size;i++){
        ...
        best_pos=optimum(store,[](const auto& candidate, const auto& max){
            return (double)(candidate.value)/(double)(candidate.weight)>(double)(max.value)/(double)(max.weight);
        });
